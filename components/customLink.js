...
const pluginDialog = {
    ...
    add: function (core) {
        ...
        /** link dialog */
        let linkDialog = this.setDialog(core);
        context.customLink.modal = linkDialog;
        context.customLink.focusElement = linkDialog.querySelector('._se_link_url');
        context.customLink.linkAnchorText = linkDialog.querySelector('._se_link_text');
        context.customLink.targetSelect = linkDialog.querySelector('.se-input-select');

        /** link controller */
        let linkController = this.setController_LinkButton(core);
        context.customLink.linkController = linkController;
        context.customLink._linkAnchor = null;

        /** add event listeners */
        linkDialog.querySelector('form').addEventListener('submit', this.submit.bind(core));
        linkController.addEventListener('click', this.onClick_linkController.bind(core));

        /** append html */
        context.dialog.modal.appendChild(linkDialog);

        /** append controller */
        context.element.relative.appendChild(linkController);

        /** empty memory */
        linkDialog = null, linkController = null;
    },
    ...
    setController_LinkButton: function (core) {
        ...
        const linkBtn = core.util.createElement('DIV');

        linkBtn.className = 'se-controller se-controller-link';
        linkBtn.innerHTML = '' +
