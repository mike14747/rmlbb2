...
export async function getInitialNewsItems() {
    const query = encodeURIComponent('{"total": count(*[_type == "newsItem"]),"newsItems": *[_type == "newsItem"] | order(date desc, title asc) [0...20]{title, date, content}}');
    return getInitialNewsData(query);
}

export async function getMoreNewsItems(start) {
    const query = encodeURIComponent(`*[_type == "newsItem"] | order(date desc, title asc) [${start}...${start + 20}]{title, date, content}`);
