...
const editTopic = async (topicId, userId, title, content) => {
    ...
    try {
        transactionResult = await session.withTransaction(async () => {
            await db
                .collection('topics')
                .updateOne({ _id: topicId }, { $set: { title, content, lastEditDate: currentDate } }, { session });

            await db
                .collection('forums')
                .updateOne({ 'lastPost.topicId': topicId, 'lastPost.replyId': { $type: 'null' } }, { $set: { lastPost: null } }, { session });
