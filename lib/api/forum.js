...
const getMostRecentPostsForHomepage = async () => {
    ...
    const data = await db
        .collection('topics')
        .find({ active: true, forumActive: true })
        .project({ title: 1, content: 1, forumName: 1, username: 1, date: 1 })
        .sort({ date: -1 })
        .limit(5)
        .map(topic => {
            topic.content = topic.content.replace(/(<([^>]+)>)/ig, '').substring(0, 50);
            topic.date = formatDateObjectWithTime(topic.date, 'short');
            return topic;
        })
        .toArray();
...
// https://www.mongodb.com/developer/how-to/use-union-all-aggregation-pipeline-stage/
